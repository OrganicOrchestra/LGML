      ...
      for (auto & tr : trackGroup.tracks) {
        if (tr->playableBuffer.getRecordedLength()) {
          ...
          if(tr->sampleChoice->selectionIsNotEmpty() ){
            File loadedFile = getEngine()->getFileAtNormalizedPath(tr->sampleChoice->getFirstSelectedValue().toString());
          ...
          if ((fp = destFile.createOutputStream())) {
            ...
            DBG("export audio : can't create output format writer");
              jassertfalse;
              continue;
            }
          }
          else{
            DBG("export audio : can't create output stream file");
            jassertfalse;
            continue;
          }
