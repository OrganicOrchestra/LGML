...
TimeManager::TimeManager() : beatTimeInSample(1000),
                             sampleRate(44100),
                             blockSize(0),
    ...
    else if (t == tapTempo)
    {
        if (!BPMLocked->boolValue())
        {
            ...
            if ((delta > 100) && (delta < 1000))
            {
                ...
                if (targetBeat - (int)targetBeat > 0.5)
                {
                    targetBeat += 1;
                }
...
{
    if (desiredTimeState.isJumping)
        return desiredTimeState.nextTime;
    else
        return timeState.nextTime;
}
...
TransportTimeInfo TimeManager::findTransportTimeInfoForLength(sample_clk_t time, double _sampleRate)
{
    ...
    // over 150 bpm (0.4s)
    while (res.beatTime < beatTimeGuessRange.getStart())
    {
        res.beatTime *= 2.0;
        res.barLength /= 2.0;
    }
    ...
    // under 70 bpm (0.85s)
    while (res.beatTime > beatTimeGuessRange.getEnd())
    {
        res.beatTime /= 2.0;
        res.barLength *= 2.0;
    }
