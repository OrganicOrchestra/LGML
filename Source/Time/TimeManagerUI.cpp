...
TimeManagerUI::TimeManagerUI(const String &contentName, TimeManager *_timeManager) : InspectableComponent(_timeManager),
                                                                                     ShapeShifterContent(this, contentName, "Be the master of Time"), timeBar(_timeManager), timeManager(_timeManager)
{
...
void TimeManagerUI::newMessage(const ParameterBase::ParamWithValue &pv)
{
    ...
    {
        if (!timeManager->linkEnabled->boolValue())
            linkEnabled->setCustomText("");
        else
        {
            linkEnabled->setCustomText(String(timeManager->linkNumPeers->intValue()) + " link peers");
        }
...
void TimeManagerUI::TimeBar::resized()
{
    ...
    for (int i = 0; i < beatPerBar; i++)
    {
        beatComponents.getUnchecked(i)->setBounds(area.removeFromLeft(beatWidth));
    }
}
...
void TimeManagerUI::TimeBar::showBeatComponents(bool show)
{
    for (int i = 0; i < beatComponents.size(); i++)
    {
        beatComponents.getUnchecked(i)->setVisible(show);
    }
}
