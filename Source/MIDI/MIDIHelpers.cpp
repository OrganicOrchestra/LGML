...
namespace MIDIHelpers
{
    ...
    MIDIIOChooser::MIDIIOChooser(MIDIListener *l, bool _showController, bool _isOutputDevice) : showController(_showController),
                                                                                                owner(l),
                                                                                                listenedIn(nullptr),
                                                                                                inP(nullptr),
    ...
    void MIDIIOChooser::enumOptionSelectionChanged(EnumParameter *ep, bool isSelected, bool isValid, const Identifier &)
    {
        ...
        else if (isValid && isSelected)
        {
            ...
            if (!isOutputDevice && autoOut && owner->hasValidInPort)
            {
                ...
                if (c)
                { //} && (lastOutDevice.isEmpty() || c->getClosestOutName(lastInDevice)==lastOutDevice)){
