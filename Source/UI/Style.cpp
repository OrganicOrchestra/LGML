...
{
public:
    ElemDrawables(Colour _crossColour, Colour _circleColour, int _crossRotation = 0) : crossColour(_crossColour), circleColour(_circleColour), crossRotation(_crossRotation)
    {
...
Drawable *AddElementButton::createDrawable(bool isHovered)
{
    ...
    if (!el)
    {
        el = new ElemDrawables(Colours::white, Colours::green, 0);
    }
...
RemoveElementButton::RemoveElementButton() : DrawableButton("Remove", DrawableButton::ButtonStyle::ImageFitted)
{
    setImages(createDrawable(), createDrawable(true));
    setPaintingIsUnclipped(true);
    setOpaque(false);
...
ElemDrawables *RemoveElementButton::getDrawables()
{
    ...
    if (!el)
    {
        el = new ElemDrawables(Colours::white, Colours::red, 45);
    }
    return el;
}
