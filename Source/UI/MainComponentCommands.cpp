        ...
        if (icl.size() > 0)
        {
            ...
            for (auto ic : icl)
            {
                ...
                if (cc != nullptr)
                {
                    ...
                    if (!ncv)
                        ncv = relatedComponent->findParentComponentOfClass<NodeContainerViewer>();
                    if (ncv && ncv->uiParams)
                    {
                    ...
                    {
                        if (ncv && ic->inspectableType == "node")
                            ncv->removeNodeUndoable(dynamic_cast<NodeBase *>(cc));
                        else if (ic->inspectableType == "controller")
                            ((Controller *)cc)->remove();
