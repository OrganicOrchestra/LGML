...
ShapeShifterContainer::ShapeShifterContainer(Direction _direction) : ShapeShifter(ShapeShifter::CONTAINER),
...
void ShapeShifterContainer::removeShifter(ShapeShifter *shifter, bool deleteShifter, bool silent, bool resizeAfter)
{
    ...
    case PANEL:
        ...
        if (deleteShifter)
            ShapeShifterManager::getInstance()->removePanel(((ShapeShifterPanel *)shifter));
...
ShapeShifterPanel *ShapeShifterContainer::insertPanelRelative(ShapeShifterPanel *panel, ShapeShifterPanel *relativeTo, ShapeShifterPanel::AttachZone zone, bool resizeAfter)
{
    ...
    case ShapeShifterPanel::AttachZone::LEFT:
        if (direction == HORIZONTAL)
            insertPanelAt(panel, shifters.indexOf(relativeTo), resizeAfter);
        else
            movePanelsInContainer(panel, relativeTo, HORIZONTAL, false);

        break;

    case ShapeShifterPanel::AttachZone::RIGHT:
        if (direction == HORIZONTAL)
            insertPanelAt(panel, shifters.indexOf(relativeTo) + 1, resizeAfter);
        else
            movePanelsInContainer(panel, relativeTo, HORIZONTAL, true);
    ...
    case ShapeShifterPanel::AttachZone::TOP:
        if (direction == VERTICAL)
            ...
            movePanelsInContainer(panel, relativeTo, VERTICAL, false);
    ...
    case ShapeShifterPanel::AttachZone::BOTTOM:
        if (direction == VERTICAL)
            ...
            movePanelsInContainer(panel, relativeTo, VERTICAL, true);
...
public:
    ...
    ShapeShifterResolver(Array<ShapeShifter *> _s, int _totalSpace, Direction _direction) : shifters(std::move(_s)),
                                                                                            direction(_direction),
...
private:
    ...
    void setLength(ShapeShifter *p, int l)
    {
        ...
        if (isHorizontal())
            p->setPreferredWidth(l);
        else
            p->setPreferredHeight(l);
    }
...
void ShapeShifterContainer::resized()
{
    ...
    {
        if (!shifters[lastNonMinimizedShifter]->isMini)
            break;
    }
    ...
    for (auto &p : shifters)
    {
        ...
        {
            if (direction == HORIZONTAL)
                p->setBounds(r.removeFromLeft(p->isMini ? miniSize : p->getPreferredWidth()));
            else
                p->setBounds(r.removeFromTop(p->isMini ? miniSize : p->getPreferredHeight()));
        }
        else
        {
            auto nonMinimizedR = r;
            if (direction == HORIZONTAL)
                nonMinimizedR.removeFromRight(trailingMinimizedShifters * miniSize);
            else
                nonMinimizedR.removeFromBottom(trailingMinimizedShifters * miniSize);
            jassert(isMini || (nonMinimizedR.getWidth() >= p->getMinWidth() && nonMinimizedR.getHeight() >= p->getMinHeight()));
            p->setBounds(nonMinimizedR);
            if (direction == HORIZONTAL)
                r.removeFromLeft(nonMinimizedR.getWidth());
            else
                r.removeFromTop(nonMinimizedR.getHeight());
        }
...
void ShapeShifterContainer::oneShifterRemaining(ShapeShifterContainer *container, ShapeShifter *lastShifter)
{
    ...
    if (lastShifter->shifterType == PANEL)
        ss = insertPanelAt((ShapeShifterPanel *)lastShifter, containerIndex, false);
    else
        ss = insertContainerAt((ShapeShifterContainer *)lastShifter, containerIndex, false);
