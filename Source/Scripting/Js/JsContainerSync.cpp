...
var getControllableForAddress(const var::NativeFunctionArgs &a)
{
    ...
    if (a.numArguments == 1)
    {
        ...
        {
            for (auto e : *a.arguments[0].getArray())
            {
                arr.add(e);
            };
        };
    ...
    else if (a.numArguments > 1)
    {
        ...
        {
            arr.add(a.arguments[i].toString());
        }
    }
    ...
    if (arr.size())
    {
        ...
        if (res)
        {
            return res->createDynamicObject();
        }
    }
...
void JsContainerSync::updateControllableNamespace(ControllableContainer *c)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (found)
                dyn->setProperty(jsNamespace[0], createDynamicObjectFromContainer(c));
            else
                DBG("proprety not found for Controllable " + c->shortName);
        }
    }
}
