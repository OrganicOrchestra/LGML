...
JsEnvironment::JsEnvironment(const String &ns, ParameterContainer *_linkedContainer) : linkedContainer(_linkedContainer),
                                                                                       localNamespace(ns),
                                                                                       localEnv(new DynamicObject()),
                                                                                       _hasValidJsFile(false),
                                                                                       _isInSyncWithLGML(false),
                                                                                       isLoadingFile(false),
                                                                                       isEnabled(true), jsParameters(new JSEnvContainer(this))
{
...
void JsEnvironment::clearListeners()
{
    ...
    while (it.next())
    {
        auto c = it.getValue();
        if (c.param.get())
            c.param->removeParameterListener(this);
    }
...
var JsEnvironment::callFunction(const String &function, const Array<var> &args, bool logResult, Result *result)
{
    ...
    if (!functionIsDefined(function))
    {
        ...
        if (result != nullptr)
            result->fail(noFunctionLogIdentifier.toString());

        if (logResult)
            NLOGE(getJsObjectAddress(), noFunctionLogIdentifier.toString());
...
Result JsEnvironment::checkUserControllableEventFunction()
{
    ...
    for (auto &f : userDefinedFunctions)
    {
        if (f.splitedName[0] == "on")
            functionNamespaces.add(&f);
    }
...
void JsEnvironment::sendAllParametersToJS()
{
    ...
    for (auto &t : listenedContainers)
    {
        if (t.get())
        {
            ...
            for (auto &c : conts)
            {
                parameterFeedbackUpdate(t, c, this);
            }
        }
    }
}
...
JSEnvContainer::JSEnvContainer(JsEnvironment *pEnv) : ParameterContainer("jsParams"), jsEnv(pEnv)
{
