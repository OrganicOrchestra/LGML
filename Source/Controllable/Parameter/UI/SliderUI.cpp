...
//==============================================================================
template <class T>
SliderUI<T>::SliderUI(ParameterBase *parameter) : ParameterUI(parameter),
...
void SliderUI<T>::resized()
{
    ...
    if (showLabel || showValue)
    {
        ...
        else
        {
            ...
            if (orientation == VERTICAL)
            { // simulate horizontal but rotate it
...
void SliderUI<T>::mouseUp(const MouseEvent &me)
{
    ...
    if (!me.mods.isLeftButtonDown())
        return;
    if (me.mods.isCommandDown() || me.mods.isShiftDown())
        return;
    ...
    if (!me.mouseWasDraggedSinceMouseDown() && !me.mods.isAnyModifierKeyDown())
    {
        if (valueBox && !valueBox->isVisible())
        {
            ...
            if (auto p = getParentComponent())
                p->addAndMakeVisible(valueBox.get());
            valueBox->showEditor();
