...
ParameterContainer::ParameterContainer(StringRef niceName) : ControllableContainer(niceName),
...
ParameterBase *ParameterContainer::addParameter(ParameterBase *p, int idxToInsert, bool doListen)
{
    if (p)
    {
        ...
        if (idxToInsert == -1)
            ...
            controllables.insert(idxToInsert, p);
...
{
    if (auto pc = dynamic_cast<ParameterContainer *>(parentContainer))
    {
        pc->dispatchFeedbackInternal(c, notifier);
    }
...
void ParameterContainer::configureFromObjectOrValues(DynamicObject *dyn, bool allowNewChilds)
{
    if (dyn)
    {
        ...
        if (dyn->hasProperty(uidIdentifier))
            uid = dyn->getProperty(uidIdentifier);

        {
            ...
            if (paramsData != nullptr)
            {
                ...
                if (hasPresetToLoad && (props.size() != 1 || hasChildConts))
                { // preset shouldnt interfere with other controllable values
                ...
                for (auto &p : props)
                {
                    ...
                    if (!c)
                    {
                        c = getControllableByShortName(p.name.toString());
                    }
                    if (c)
                    {
