...
FastMapper::FastMapper(StringRef name) : ParameterContainer(name),
...
bool FastMapper::checkValidNewFastMap(ParameterBase *referenceInParam, ParameterBase *referenceOutParam, bool warnDups)
{
    ...
    Array<ParameterProxy *> refInConflicts, refOutConflicts;
    ...
    for (auto &ff : maps)
    {
        ...
        if (warnDups)
        {
            ...
            {
                refInConflicts.add(ff->referenceOut);
                continue;
                ...
                //     return false;
                // }
                // else if (cancelAddReplace == 2)
                // {
                //     ff->referenceOut->setParamToReferTo(referenceOutParam);
                //     return false;
                // }
                // else if (cancelAddReplace == 1)
                // {
                //     warnDups = false;
                // }
            }
            ...
            {
                refOutConflicts.add(ff->referenceIn);
                continue;
            }
    ...
    if (refInConflicts.size() || refOutConflicts.size())
    {
        auto checkConflicts = [](ParameterBase* refParam, Array<ParameterProxy*> conflicts) {
            ...
            // String otherAddress = otherP?otherP->getControlAddress().toString():"None";
            String infoText = juce::translate("parameter mapped 123\n is already mapped to 456 \n ?re you sure to proceed ? ").replace("123", fromAddr).replace("456", addrList.joinIntoString(", "));
            int cancelAddReplace = AlertWindow::showYesNoCancelBox(AlertWindow::AlertIconType::QuestionIcon,
                                                                   "FastMapWarning",
                                                                   infoText, juce::translate("Add Anyway!"), //1
                                                                   juce::translate("Replace All"),           // 2
                                                                   juce::translate("Cancel"),                // 0
                                                                   nullptr,                                  //associatedComponent
                                                                   nullptr);                                 //callback

            if (cancelAddReplace == 2)
            {
                std::for_each(conflicts.begin(), conflicts.end(), [refParam](auto e) {
                    if (e)
                    {
                        e->setParamToReferTo(refParam);
                    }
                    else
                    {
                        jassertfalse;
                ...
                // ff->referenceIn->setParamToReferTo(referenceInParam);return false;
            }
