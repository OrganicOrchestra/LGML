...
void FloatSliderUI::mouseUp(const MouseEvent &me)
{
  ...
  if (me.getNumberOfClicks() >= 2) {
    ...
    if(parameter->isUserDefined){
      ...
      nameWindow.addTextEditor("minValue", parameter->minimumValue);
      nameWindow.addTextBlock("max Value");
      nameWindow.addTextEditor("maxValue", parameter->maximumValue);
    }
...
{
  parameter->setNormalizedValue(jmin(jmax(value,0.0f),1.0f));
}
